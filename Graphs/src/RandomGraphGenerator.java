...
public class RandomGraphGenerator {
    ...
    public RandomGraphGenerator(String type)
    {
        this.vertices = random.nextInt(MAX_LIMIT) + 1;
        this.edges = random.nextInt(computeMaxSparseEdges(vertices)) + 1;
        adjacencyList = new ArrayList<>(vertices);
        for (int i = 0; i < vertices; i++)
            adjacencyList.add(new ArrayList<>());
    ...
    int computeMaxSparseEdges(int numOfVertices)
    {
        // As it is a directed graph
        // So, for a given number of vertices
        // there can be at-most v*(v-1) number of edges
    ...
    public static void main(String[] args) {
        RandomGraphGenerator randomGraphGenerator = new RandomGraphGenerator("sparse");
